...
const std::string local_address("/etc/local_address");
const std::string mapped_address("/etc/mapped_address");
const std::string maping_behavior("/etc/mapping_behavior");
const std::string filtering_behavior("/etc/filtering_behavior");
int WriteResultToFile(std::string filename, std::string result) {
	FILE *pfile;
	int ret;
	...
	ret = fwrite(result.c_str(), 1, result.length(), pfile);
	printf("%s set to: \"%s\" - %d\n", filename.c_str(), result.c_str(), ret);
...
void DumpResults(StunClientLogicConfig& config, StunClientResults& results)
{
    ...
    if (results.fBindingTestSuccess)
    {
        ...
        WriteResultToFile(local_address, std::string(szBuffer));

        results.addrMapped.ToStringBuffer(szBuffer, buffersize);
        Logging::LogMsg(LL_ALWAYS, "Mapped address: %s", szBuffer);

        WriteResultToFile(mapped_address, std::string(szBuffer));
    }
    ...
    if (config.fBehaviorTest)
    {
        ...
        if (results.fBehaviorTestSuccess)
        {
            ...
            WriteResultToFile(maping_behavior, strResult);
        }
    }
    ...
    if (config.fFilteringTest)
    {
        ...
        if (results.fFilteringTestSuccess)
        {
            ...
            WriteResultToFile(filtering_behavior, strResult);
        }
    }
}
...
int stunclient(int argc, char** argv)
{
